...
function mastodon(cb) {
  pump(
    [
      gulp.src(['node_modules/moment/min/moment.min.js', 'src/mastodon.js']),
      uglify(),
...
function packages(cb) {
  pump(
    [
      gulp.src(['node_modules/axios/dist/axios.min.js', 'src/packages.js']),
      uglify(),
...
function images() {
  return gulp.src('src/images/**/*').pipe(imagemin()).pipe(gulp.dest('assets/images'))
}

function imagesResize() {
  return gulp
    .src('src/images/central-management/*')
    .pipe(
      imageResize({
        width: 675,
        height: 480,
